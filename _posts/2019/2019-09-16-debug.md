...
### 2. 执行命令jdb DisplayJDB开始调试程序，我们可以用help先看看jdb都有哪些调试命令，如下图：
...
### 4. 继续执行，单步执行太慢了，可以使用 ```stop at DisplayJDB:14```在14行处下个断点，
然后使用```step up```执行到下一断点处，继续执行进入到add方法,add执行结束后回到test方法。
...
### 5. 接下来是一个for循环，```stop``` 查看当前断点情况，在21行新下一个断点，然后使用```cont```执行，
